...
SELECT name FROM Models WHERE id BETWEEN 15 and 25 ORDER BY name;
...
-- Inner Joins, or just "Joins" are good for when you want data that match two pieces of 
-- information from two different tables. The pieces of data that do not have a match will
-- not be included in the output. A left join adds the data from a specified "left" table
-- that regardless of whether or not it matches up with a corresponding piece of data in the
-- other table.
...
-- before: 
    -- SELECT b.name,
    --        m.name,SELECT name FROM Models WHERE id BETWEEN 15 and 25 ORDER BY name;
    --        m.year,
    --        b.discontinued
    -- FROM Models AS m
    --   LEFT JOIN brands AS b
...
VALUES ("IIHS Safety Award", 2015, (SELECT id FROM Models WHERE name = "Malibu"));

INSERT INTO AWARDS (name, year, winner) 
VALUES ("IIHS Safety Award", 2015, (SELECT id FROM Models WHERE name = "Outback"));
